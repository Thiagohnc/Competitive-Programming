/* Problem: http://www.spoj.com/problems/ILKQUERYIII/ */
...
struct BitmapRank {
	...
	void swap(int i, int val) {
		int temp = !!getBit(i);
		setBit2(i,getBit(i+1));
		setBit2(i+1,temp);
		if(i/bits != (i+1)/bits) {
			if(val == 1) count[i/bits + 1]--;
			else count[i/bits + 1]++;
		}
	}
...
int main(void) {
	...
	for(int z = 0; z < q; z++) {
		...
		if(qt == 0) {
			...
			scanf("%d", &qt);
			if(qt == n - 1) continue;
			vi::iterator it = lb(idx[v2[qt]].begin(), idx[v2[qt]].end(), qt);
			*it += 1;
			it = lb(idx[v2[qt+1]].begin(), idx[v2[qt+1]].end(), qt+1);
			*it -= 1;
			wv.swap(qt);
			int temp = v2[qt+1];
			v2[qt+1] = v2[qt];
